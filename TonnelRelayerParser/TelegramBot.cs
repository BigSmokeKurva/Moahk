...
public class TelegramBot : IDisposable
{
        ...
        Data.Entities.User user)
    {
        var msgText = """
                      ...
                      - Сравнение с максимумом за 2 недели - сравнивает с самой высокой ценой в activity за период ( самый рисковый способ но количество подарков будет находиться больше)
                      - 75-й процентиль - статистический метод оценки ( так же сравнивает с ценой в activity но не с самой высокой ценой а по формуле 75процентиля)
                      - Разница со вторым по полу - сравнивает с вторым по дешевизне подарком ( БЕЗ УЧЕТА ФОНА!)
